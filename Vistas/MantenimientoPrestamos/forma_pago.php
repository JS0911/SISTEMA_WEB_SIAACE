...
$permisosFormaPago = new PermisosUsuarios();
...
$permisos2 = $permisosFormaPago->get_Permisos_Usuarios($id_rol, $id_objeto_Cuentas);
...
<head>
    ...
    <title>Mantenimiento Forma de Pago</title>
...
<body class="sb-nav-fixed">
    ...
    <div id="layoutSidenav">
                        ...
                        if (!empty($permisos2) && $permisos2[0]['PERMISOS_CONSULTAR'] == 1) {
                         ...
                         <div class="sb-sidenav-collapse-arrow"><i class="fas fa-angle-down"></i></div>
                            ...
                            if (!empty($permisos2) && $permisos2[0]['PERMISOS_CONSULTAR'] == 1) {
                                ...
                                echo '<a class="nav-link" href="../MantenimientoCuentas/forma_pago.php"><i class="fa fa-credit-card" aria-hidden="true"></i><span style="margin-left: 5px;"> Forma de Pago</a>';
                            }
                            echo '</nav>';
                            echo '</div>';
                        }
        ...
        <div id="layoutSidenav_content">
            ...
            <!-- DESDE AQUI COMIENZA EL MANTENIMIENTO DE FORMA DE PAGO -->
            <main>
                <div class="container-fluid">
                    <!-- Botón para abrir el formulario de creación -->
                    <div class="container" style="max-width: 1400px;">
                        <center>
                            <h1 class="mt-4 mb-4">Mantenimiento Forma de Pago</h1>
                        </center>
                            ...
                            <div class="modal-body">
                                ...
                                <form>
                                    <div class="form-group">
                                        ...
                                        <input type="text" maxlength="15" class="form-control" id="agregar-pago" required pattern="^(?!\s)(?!.*\s$).*$" title="No se permiten espacios en blanco ni campo vacío" oninput="this.value = this.value.toUpperCase()">
                                        <div id="mensaje1"></div>
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary" id="btn-editar" onclick="updateFormaPago()" disabled>Guardar</button>
                            </div>
                        </div>
                    </div>
                </div>
            </main>
            <!-- AQUI FINALIZA EL MANTENIMIENTO DE FORMA DE PAGO -->
    ...
    <script>
        ...
        function Lista_Forma_Pago() {
            ...
            fetch('http://localhost:90/SISTEMA_WEB_SIAACE/Controladores/forma_pago.php?op=GetFpagos', {
                    method: 'GET',
                    headers: {
        ...
        function Insertar_Forma_Pago() {
            $("#btn-agregar").click(function() {
                ...
                if (forma_pago == "" || descripcion == "" || estado == "") {
                    ...
                    // Crear una forma de pago con los datos a enviar al servidor
                    var datos = {
                    ...
                    fetch('http://localhost:90/SISTEMA_WEB_SIAACE/Controladores/forma_pago.php?op=InsertFpago', {
                            method: 'POST',
                            headers: {
                        ...
                        .then(function(response) {
                            if (response.ok) {
                                if (response.status === 200) {
                                    ...
                                    // Si el código de respuesta es 409 (Conflict), muestra mensaje de forma de pago existente
                                    return response.json().then(function(data) {
                                        console.log(data);
        ...
        function cargarFormaPago(id) {
            // Crear una forma de pago con el ID de forma de pago
            var data = {
                "ID_FPAGO": id
            };

            // Realiza una solicitud FETCH para obtener los detalles de la forma de pago por su ID
            fetch('http://localhost:90/SISTEMA_WEB_SIAACE/Controladores/forma_pago.php?op=GetFpago', {
                    method: 'POST',
                    headers: {
                        'Accept': 'application/json',
                        'Content-Type': 'application/json'
                ...
                .then(function(pago) {
                    ...
                    document.getElementById('editar-id-pago').value = pago.ID_FPAGO;
                    document.getElementById('editar-pago').value = pago.FORMA_DE_PAGO;
                    document.getElementById('editar-descripcion').value = pago.DESCRIPCION;
                    document.getElementById('editar-estado').value = pago.ESTADO;
                })
                .catch(function(error) {
                    // Manejar el error aquí
        ...
        function updateFormaPago() {
            var id_pago = document.getElementById('editar-id-pago').value;
            var pago = document.getElementById('editar-pago').value;
            var descripcion = document.getElementById('editar-descripcion').value;
            var estado = document.getElementById('editar-estado').value;

            if (pago == "" || descripcion == "" || estado == "") {
                Swal.fire({
                    icon: 'error',
                    title: 'Error!',
                    text: 'No se pueden enviar Campos Vacios.'
                ...
                // Realiza una solicitud FETCH para actualizar los datos de la forma de pago
                fetch('http://localhost:90/SISTEMA_WEB_SIAACE/Controladores/forma_pago.php?op=UpdateFpago', {
                        method: 'POST',
                        headers: {
                            'Accept': 'application/json',
                            'Content-Type': 'application/json'
                    ...
                    .catch(function(error) {
                        ...
                        Swal.fire({
                            icon: 'error',
                            title: 'Error',
                            text: 'Error al actualizar los datos de la forma de pago: ' + error.message
                        });
                    });
            }
        }
        ...
        function eliminarFormaPago(id_pago) {
            Swal.fire({
                title: '¿Estás seguro?',
                ...
                if (result.isConfirmed) {
                    fetch('http://localhost:90/SISTEMA_WEB_SIAACE/Controladores/forma_pago.php?op=EliminarFpago', {
                            method: 'POST',
                            headers: {
                                'Accept': 'application/json',
                                'Content-Type': 'application/json'
                        ...
                        .then(function(response) {
                            if (response.ok) {
                                ...
                                Swal.fire('Forma de Pago eliminado', '', 'success')
                                    .then(() => {
                                        // Recargar la página para mostrar los nuevos datos
                                        location.reload();
                                        // Recargar la lista de forma de pago después de eliminar
                                        //Lista_Forma_Pago();
                                    });
                            } else {
                        ...
                        .catch(function(error) {
                            // Manejar el error aquí
                            Swal.fire('Error', 'Error al eliminar la forma de pago: ' + error.message, 'error');
                        });
                }
            });
        }
        ...
        function validarNombre() {
            ...
            handleInputAndBlurEvents(nombreFormaPago, expresionValidadora1, mensaje1, "Solo se permiten Letras Mayúsculas");
    ...
    <script>
        ...
        $('#agregar-pago, #editar-pago').on('input', function() {
            var input = $(this);
